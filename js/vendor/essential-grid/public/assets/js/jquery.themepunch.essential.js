		...
		tpessential: function(options) {
				...
				return this.each(function() {
					...
					if (opt.get!="true" && opt.get!=true) {
						...
						if (opt.firstshowever==undefined) jQuery(opt.filterGroupClass+'.anularebutonn,'+opt.filterGroupClass+' .anularebutonn').css({visibility:"hidden"});
						...
						************************************/
						...
						if (opt.loadMoreType=="button") {
							container.append('<div class="esg-loadmore-wrapper" style="text-align:center"><div class="anularebutonn esg-loadmore">LOAD MORE</div></div>');
						...
						if (opt.loadMoreType=="scroll") {
							...
							container.append('<div style="display:inline-block" class="anularebutonn esg-loadmore">LOAD MORE</div>');
 ...
 function setItemsOnPages(opt) {
		...
		if (opt.evenCobbles == "on" && opt.cobblesPattern!=undefined) {
		 ...
		 var minindex = itemperpage*opt.currentpage,
		 	 ...
		 	 filters = jQuery(opt.filterGroupClass+'.esg-filterbutton.selected:not(.anularebutonn),'+opt.filterGroupClass+' .esg-filterbutton.selected:not(.anularebutonn)'),
		 	 indexcounter = 0;
		 	...
		 	if (opt.currentpage<3) {
			 	for (var i=0;i<4;i++) {
				  if (i==opt.currentpage)
		 			...
		 			extraclass="";
		 		  opt.maxpage++;
			 	  paginholder.append('<div class="anularebutonn esg-filterbutton esg-pagination-button '+extraclass+'" data-page="'+i+'">'+(i+1)+'</div>');
			 	}
			 	paginholder.append('<div class="anularebutonn">...</div>');
			 	paginholder.append('<div class="anularebutonn esg-filterbutton esg-pagination-button '+extraclass+'" data-page="'+(pageamounts-1)+'">'+(pageamounts)+'</div>');
		 	}

		 	else

		 	if (pageamounts - opt.currentpage<4) {
			 	paginholder.append('<div class="anularebutonn esg-filterbutton esg-pagination-button '+extraclass+'" data-page="0">1</div>');
			 	paginholder.append('<div class="anularebutonn">...</div>');
	 	...
	 	if (opt.maxpage==1) {
		 	 jQuery(opt.filterGroupClass+'.anularebutonn,'+opt.filterGroupClass+' .anularebutonn').not('.esg-loadmore').css({display:'none'});

		 	 paginholder.css({display:'none'});
	 	} else {
		 	 jQuery(opt.filterGroupClass+'.anularebutonn,'+opt.filterGroupClass+' .anularebutonn').css({display:'inline-block'});
		...
		if (opt.firstshowever==undefined) {
			if (container.is(":hidden")) {
				...
				setTimeout(function() {
					...
					jQuery(opt.filterGroupClass+'.anularebutonn, '+opt.filterGroupClass+' .anularebutonn').css({visibility:"visible"});
				},300)
			} else {
				runGridMain(opt,newelementadded);
				jQuery(opt.filterGroupClass+'.anularebutonn, '+opt.filterGroupClass+' .anularebutonn').css({visibility:"visible"});
